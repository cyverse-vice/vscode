...
# Load hidden .env files from K8s CSI driver mounts
# These files are mounted by Kubernetes CSI driver into user home directory
if [ -f "$HOME/.env" ]; then
    echo "Loading environment variables from $HOME/.env"
    export $(grep -v '^#' "$HOME/.env" | xargs -d '\n')
fi

# Support multiple .env files with different names
...
# add mcp server
echo '{{"mcpServers":{"filesystem":{"disabled":false,"timeout":60,"type":"stdio","command":"npx","args":["-y","@modelcontextprotocol/server-filesystem","/home/vscode/data-store/","/config/"],"autoApprove":["read_file","read_multiple_files","write_file","edit_file","create_directory","list_directory","list_directory_with_sizes","directory_tree","move_file","search_files","get_file_info","list_allowed_directories"]},"datastore":{"timeout":60,"type":"streamableHttp","url":"http://mcp.cyverse.ai/mcp","autoApprove":["ds_directory_tree","ds_get_file_info","ds_get_metadata","ds_list_allowed_directories","ds_list_directory","ds_list_directory_details","ds_read_file","ds_search_files","ds_search_metadata"]}}}' > $HOME/.claude.json
